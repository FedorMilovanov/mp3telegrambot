...
1. ГРАНИЦА ПРОДУКТОВ
«Конспект» — максимально дословная стенограмма; не пересказывай и не улучшай его.
Study Analysis даёт только новую исследовательскую ценность, не второй конспект.

2. КЛЮЧЕВЫЕ ПОНЯТИЯ
Выбирай 2–5 узлов, а не словарный список. Для каждого: богословский вопрос;
что утверждается и отрицается; отличие от ближайшей подмены; точная роль в
аргументе ЭТОГО материала; опора на Писание/цитату/таймкод; что поставлено на кон.
Общеизвестное слово без небанального различения опусти. Блок, переносимый без
изменений в другую проповедь, является наполнителем и удаляется.

3. КЛЮЧЕВЫЕ СЛОВА В КОНТЕКСТЕ ПИСАНИЯ
Норма 0–3; ноль лучше декоративного греческого/еврейского. Используй только
structured block type="word_study" и поля: scripture_ref, russian_quote,
russian_focus, original_form, lemma, transliteration, russian_pronunciation,
grammar, basic_meaning, meaning_in_context, role_in_argument, limits_of_claim,
source, anchor_timestamp. Точный стих, русская фраза, форма в стихе, лемма,
произношение, источник и таймкод обязательны. Разделяй словарное значение,
контекст стиха, роль в проповеди и применение; применение не является значением
слова. Укажи, чего одно слово само по себе не доказывает. Нет точной пользы —
блок не создавай.

4. НЕИЗМЕНЯЕМАЯ STUDY-СТРУКТУРА
Заголовок дословно: «Заблуждения и ответ ортодоксии». Только при реальном
основании, 1–3 пары. Каждая пара — два отдельных абзаца:

**Название богословской проблемы** ❌ **Подмена: название заблуждения.** ...

✅ **Ответ ортодоксальной церкви.** ...

Не склеивай, не переименовывай и не снимай маркеры на Study. В ответе нужны
конкретные основания Писания/Собора/Синода/исповедания и таймкод при наличии.
ReflectionApplication не копирует этот Study-only формат.

5. ФИНАЛЬНЫЙ ОТБОР
Удали общие, неподтверждённые, повторяющие Конспект и имитирующие глубину блоки.
"""
