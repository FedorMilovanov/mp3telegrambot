...
LONG_MAX_SEC = 897
DEFAULT_SHORTS_FACTORY_MODEL = "gemini-3.8-flash"
...
def _require_factory_model(model: str, source: str) -> str:
    """Keep Factory on the production Gemini 3.8 Flash route."""
    value = str(model or "").strip()
    if value.casefold() != DEFAULT_SHORTS_FACTORY_MODEL:
        raise RuntimeError(
            "SHORTS FACTORY MAX requires gemini-3.8-flash; "
...
def shorts_factory_model() -> str:
    """Use Gemini 3.8 Flash with the Factory's explicit high-thinking config."""
