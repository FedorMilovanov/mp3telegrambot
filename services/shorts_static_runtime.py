...
_INSTALLED = False
_CACHE: dict[tuple[str, int, int, int, int], bool] = {}
_CACHE_LIMIT = 256
...
def _classify_static_metrics(
    *,
    ...
    values = [float(v) for v in ydif_values if math.isfinite(float(v))]
...
def _probe_filter(noise_db: float, freeze_seconds: float) -> str:
    ...
    return (
        ...
        "scale=96:96:flags=area,format=gray,setpts=PTS-STARTPTS,"
        f"freezedetect=n={noise_db:.1f}dB:d={freeze_seconds:.2f},"
        "signalstats,metadata=mode=print"
    )


def _cache_key(video_path: Path, sample_start: float, probe_seconds: float) -> tuple[str, int, int, int, int]:
    try:
...
async def _is_static_video_confident(
    ...
    keep the existing crop_zoom behavior. ``True`` is returned only when the
    low-resolution central motion probe strongly supports a static slide.
    """
    if os.getenv("SHORTS_STATIC_BLUR_AUTO", "1").strip().lower() in {
        "0", "false", "no", "off",
    }:
        return False
    ...
    start = max(0.0, float(sample_start) + start_offset)

    key = _cache_key(video_path, start, probe)
    ...
    if proc.returncode != 0:
        logger.warning(
            "Short static detector ffmpeg rc=%s — keep crop_zoom: %s",
            proc.returncode,
            (proc.stderr or "")[-240:],
        )
        return False

    freeze_ratio = _freeze_coverage(output, probe)
    ydif_values = _parse_ydif_values(output)
    is_static, metrics = _classify_static_metrics(
        freeze_ratio=freeze_ratio,
        ydif_values=ydif_values,
        probe_seconds=probe,
    )

    logger.info(
        "Short visual classifier: %s at %.1fs probe=%.1fs freeze=%.3f "
        "YDIF median=%s p90=%s p98=%s samples=%s reason=%s",
        "static→full_frame_blur" if is_static else "moving/uncertain→crop_zoom",
        start,
        probe,
...
def install_short_static_runtime() -> str:
    ...
    _INSTALLED = True
    return "moving=crop_zoom; confident static slide=full_frame_blur; errors keep crop"
