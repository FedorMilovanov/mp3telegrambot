...
def parse_srt_text(text: str) -> list[pipeline.Cue]:
    ...
    for block_number, block in enumerate(blocks, start=1):
        ...
        match = _TIMING_RE.match(lines[timing_index].replace("\u200b", ""))
        if not match:
...
def normalize_srt_cues(cues: list[pipeline.Cue], duration: float) -> tuple[list[pipeline.Cue], list[str]]:
    ...
    for original_index, cue in ordered:
        ...
        if start >= duration:
            raise RuntimeError(f"Реплика SRT #{number} начинается после конца видео ({start:.3f} >= {duration:.3f}).")
        if end <= start:
...
def group_srt_cues(cues: list[pipeline.Cue], *, target_seconds: float = 7.5, max_seconds: float = 12.5) -> list[dict[str, Any]]:
    ...
    def flush() -> None:
        ...
        if not current:
            return
        groups.append({"id": len(groups) + 1, "start": float(current[0].start), "end": float(current[-1].end), "source": " ".join(item.text for item in current).strip()})
        current = []
    ...
    for cue in cues:
        ...
        if gap > 1.15 or prospective > max_seconds or (sentence_end and current_duration >= target_seconds * 0.68):
            flush()
            current = [cue]
        else:
            current.append(cue)
    ...
    if len(groups) >= 2:
        tail = groups[-1]
        previous = groups[-2]
        if tail["end"] - tail["start"] < 2.2 and tail["end"] - previous["start"] <= max_seconds * 1.20:
            groups[-2] = {"id": previous["id"], "start": previous["start"], "end": tail["end"], "source": f"{previous['source']} {tail['source']}".strip()}
            groups.pop()
...
def _build_direct_segments(groups: list[dict[str, Any]], *, delay_ms: int, duration: float) -> tuple[list[dict[str, Any]], list[pipeline.Cue]]:
    ...
    for index, group in enumerate(groups, start=1):
        ...
        render_end = min(max(start + 0.35, source_end - effective_delay), latest_render_end)
        if render_end <= start:
        ...
        segments.append({"id": index, "start": round(start, 3), "end": round(render_end, 3), "start_delay_ms": effective_delay_ms, "reference_profile": profile, "tail_guard": 0.36 if profile == "extended" else 0.42, "text": text, "source_end": round(source_end, 3), "source": text})
...
def main() -> None:
    ...
    source_dir, input_dir, output_dir = root / "source", root / "input", root / "output"
    ...
    parsed_cues = parse_srt_text(uploaded_srt.read_text(encoding="utf-8-sig"))
    log("=== 2. SOURCE / YOUTUBE ===")
    source = source_dir / "source.mp4"
    source_url, video_id = str(request["source_url"]), str(request["video_id"])
    ...
    russian_title = generate_russian_title(metadata, groups, model_name=str(request.get("title_model") or "gemini-3.5-flash-lite"), video_id=video_id)
    (root / "russian_title.txt").write_text(russian_title + "\n", encoding="utf-8")
    update_project_record(project_id, title=russian_title, source_url=source_url, work_root=root, metadata={"video_id": video_id, "translation_mode": "direct", "russian_title": russian_title, "translation_source": "user_ready_srt", "srt_cues": len(parsed_cues), "render_groups": len(groups)})
    delay_ms = int(request.get("russian_delay_ms") or 420)
    render_segments, shifted_cues = _build_direct_segments(groups, delay_ms=delay_ms, duration=duration)
    segments_json, final_srt = root / "segments_ru_final.json", output_dir / "russian_subtitles.srt"
    translation_txt, uploaded_copy = output_dir / "russian_translation.txt", output_dir / "ready_translation_original.srt"
    ...
    stable_mixed, stable_russian = output_dir / "final_upload.mp4", output_dir / "russian_only.mp4"
    log("=== 4. VOXCPM2 / ГОТОВЫЙ ТЕКСТ БЕЗ GEMINI-ПРАВОК ===")
    russian_timeline = _run_voxcpm_and_master(root=root, request=request, source=source, cues=normalized_cues, duration=duration, segments_json=segments_json, final_mixed=stable_mixed, final_russian=stable_russian)
    ...
    for source_path, destination in ((stable_mixed, named_mixed), (stable_russian, named_russian), (final_srt, named_srt), (translation_txt, named_translation), (uploaded_copy, named_original_srt)):
        _hardlink_or_copy(source_path, destination)
    qa_path = output_dir / "translation_qa.txt"
    qa_lines = ["Режим: пользовательский готовый SRT.", "Gemini не переводил, не проверял, не сокращал и не переписывал русский текст.", f"Исходных SRT-реплик: {len(parsed_cues)}.", f"Речевых блоков VoxCPM2: {len(render_segments)}.", f"Технических корректировок таймкодов: {len(timing_adjustments)}."]
    if timing_adjustments:
        qa_lines.extend(["", *timing_adjustments])
    qa_path.write_text("\n".join(qa_lines).rstrip() + "\n", encoding="utf-8")
    manifest = {"schema_version": 3, "phase": "completed", "project_id": project_id, "video_id": video_id, "source_url": source_url, "original_title": metadata.get("title") or "", "russian_title": russian_title, "channel": metadata.get("uploader") or metadata.get("channel") or "", "duration_seconds": round(duration, 4), "translation_mode": "direct", "translation_model": "user", "translation_passes": 0, "translation_rewritten": False, "original_level": float(request.get("original_level") or 0.18), "russian_delay_ms": delay_ms, "srt_cues": len(parsed_cues), "render_segments": len(render_segments), "timing_adjustments": timing_adjustments, "outputs": {"mixed": str(named_mixed), "russian_only": str(named_russian), "russian_srt": str(named_srt), "uploaded_srt": str(named_original_srt), "translation": str(named_translation), "qa": str(qa_path), "russian_timeline": str(russian_timeline)}, "telegram_outputs": [_telegram_entry(named_mixed, filename=named_mixed.name, label="Готовый ролик: ваш SRT без изменений, оригинал 18%", primary=True, video=True), _telegram_entry(named_russian, filename=named_russian.name, label="Версия только с русским голосом", video=True, send_default=False), _telegram_entry(named_srt, filename=named_srt.name, label="Финальные русские субтитры с задержкой 420 мс"), _telegram_entry(named_original_srt, filename=named_original_srt.name, label="Ваш исходный готовый SRT"), _telegram_entry(named_translation, filename=named_translation.name, label="Ваш русский текст по речевым блокам"), _telegram_entry(qa_path, filename=f"{russian_title} — технический отчёт.txt", label="Технический отчёт без проверки перевода")]}
