...
def read_segments(path: Path) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(payload, start=1):
        ...
        segment["tail_guard"] = float(segment.get("tail_guard", 0.32))
        segment["start_delay_ms"] = int(segment.get("start_delay_ms", 0))
        segment["reference_profile"] = str(
            segment.get("reference_profile", "extended")
        )

        if segment["end"] <= segment["start"]:
            raise RuntimeError(f"Некорректный сегмент #{segment['id']}.")
        if segment["start"] < previous_end - 0.001:
            raise RuntimeError(f"Пересечение у сегмента #{segment['id']}.")
        if not segment["text"]:
            raise RuntimeError(f"Пустой текст #{segment['id']}.")
        if segment["reference_profile"] not in {"extended", "composite"}:
            raise RuntimeError(
                f"Неизвестный reference_profile у #{segment['id']}."
            )
...
def clean_tail_restart(
    ...
    # Fade only inside the already detected silence, never over speech.
...
def candidate_score(
    ...
    if ratio < 0.48:
        score += 70.0 + (0.48 - ratio) * 100.0
    elif ratio > 1.50:
        score += 50.0 + (ratio - 1.50) * 50.0

    score += float(candidate["clipping_ratio"]) * 5000.0

    leading = float(candidate["leading_silence"])
    if leading > 0.55:
        score += (leading - 0.55) * 18.0

    # Prefer natural candidates that fit without extreme correction.
...
def build_timeline(
    ...
    for index, (segment, _) in enumerate(fitted_segments):
        delay_ms = int(round(float(segment["start"]) * 1000.0)) + int(segment.get("start_delay_ms", 0))
        label = f"s{index}"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Production VoxCPM2 CPU render with NoChew candidate selection."
    )
    ...
    log("Reference-only; B для 1–3; C для финала")
    log("Два кандидата всегда; третий только при необходимости")
    log("Короткие результаты не замедляются")
    ...
    for position, segment in enumerate(segments, start=1):
        ...
        log(
            ...
            f"{profile.upper()} / {target_duration:.2f} сек. / "
        ...
        while attempt_index <= 3:
            if attempt_index == 3:
                ...
                need_third = (
                    bool(best_so_far["tail_info"].get("suspicious"))
                    or float(best_so_far["duration"]) < speech_slot * 0.48
                    or float(best_so_far["clipping_ratio"]) > 0.0005
                )
                if not need_third:
                    break
            ...
            log(
                ...
                f"tail_restart={tail_info.get('suspicious', False)}; "
        ...
        if float(fit["tempo"]) > 1.50:
            raise RuntimeError(
                f"Сегмент #{segment_id}: требуется слишком сильное "
                f"ускорение atempo={fit['tempo']:.3f}"
            )
    ...
    report = {
        "schema_version": "4.0-final",
        "strategy": (
            "NoChew + reference-only + B extended for segments 1-3 + "
            "C composite for final segment + multi-candidate selection"
        ),
