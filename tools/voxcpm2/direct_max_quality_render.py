...
def fit_without_slowdown(clean_path: Path, fitted_path: Path, target_duration: float, tail_guard: float) -> dict[str, Any]:
    ...
    run_checked([
        "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(clean_path),
        "-af", ",".join(filters), "-ar", str(EXPECTED_OUTPUT_SR), "-ac", "1",
        "-c:a", "pcm_s24le", str(fitted_path),
    ])
    return {
...
def build_timeline(fitted_segments: list[tuple[dict[str, Any], Path]], output: Path, total_duration: float) -> None:
    ...
    filters.append(
        ...
        + "highpass=f=35,alimiter=limit=0.985[out]"
    )
    command.extend([
        "-filter_complex", ";".join(filters), "-map", "[out]",
        "-ar", str(EXPECTED_OUTPUT_SR), "-ac", "2", "-c:a", "pcm_s24le", str(output),
    ])
    run_checked(command)
