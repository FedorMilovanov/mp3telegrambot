...
if ($RunningEditors.Count -gt 0) {
    throw "Close GPU-heavy editors before the recovered-session test"
}
...
$PreflightStdout = Join-Path $RunDir "recovery_preflight.stdout.txt"
$PreflightStderr = Join-Path $RunDir "recovery_preflight.stderr.txt"
...
Write-Host "RTX 3060 VOXCPM2 CUDA ZERO-SHOT — RECOVERED SESSION" -ForegroundColor Cyan
Write-Host "Old Event ID 153 records are ignored; only new RecordIds count" -ForegroundColor Yellow
Write-Host "A fresh isolated CUDA context and memory round-trip run first" -ForegroundColor Yellow
Write-Host "One zero-shot phrase, one inference step, one attempt" -ForegroundColor Yellow
...
try {
    ...
    Show-GpuTelemetry "GPU before recovery preflight"
...
assert float(y[123456].item()) == 123456.0
print("RECOVERY PREFLIGHT PASSED")
'@
    $PreflightOutput = @(& $CudaPython -c $PreflightCode 2>&1)
    $PreflightExitCode = $LASTEXITCODE
    [IO.File]::WriteAllLines($PreflightStdout, $PreflightOutput, [Text.Encoding]::UTF8)
    [IO.File]::WriteAllText($PreflightStderr, "", [Text.Encoding]::UTF8)

    Start-Sleep -Seconds 5
    $PreflightEvents = Get-NewGpuEvents -AfterRecordId $BaselineRecordId -AfterTime $StartedAt
    if ($PreflightEvents.Count -gt 0) {
        $PreflightEvents |
            Sort-Object RecordId |
            Select-Object TimeCreated, RecordId, ProviderName, Id, LevelDisplayName, Message |
            Format-List
        throw "New GPU/Display/WHEA event appeared during recovery preflight"
    }
    if ($PreflightExitCode -ne 0) {
        $PreflightOutput | ForEach-Object { Write-Host $_ -ForegroundColor Red }
        throw "Recovery preflight failed with exit code $PreflightExitCode"
    }
    Write-Host "RECOVERY PREFLIGHT PASSED — starting one zero-shot attempt" -ForegroundColor Green
    ...
    if ($GpuEvents.Count -gt 0) {
        ...
        $GpuEvents |
            ...
            Select-Object TimeCreated, RecordId, ProviderName, Id, LevelDisplayName, Message |
            Format-List
        throw "Windows logged a new GPU/Display/WHEA event during recovered-session zero-shot"
    }
    if (-not $Completed) {
    ...
    Write-Host "" 
    Write-Host "VOXCPM2 CUDA RECOVERED-SESSION ZERO-SHOT PASSED" -ForegroundColor Green
