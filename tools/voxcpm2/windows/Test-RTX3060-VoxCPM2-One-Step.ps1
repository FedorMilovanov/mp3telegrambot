...
param(
    ...
    [string]$ReferenceWav = "C:\AI-Archive\RTX3060-VOXCPM2-VIDEO-SMOKE\Z20Py4yQhYQ\reference\reference_16k_mono.wav",
    [string]$WorkRoot = "C:\AI-Archive\RTX3060-VOXCPM2-ONE-STEP",
    [string]$Text = "Тест.",
...
foreach ($Required in @($CudaPython, $ArchiveRoot, $ReferenceWav, $Probe)) {
...
if ($RunningEditors.Count -gt 0) {
    throw "Close GPU-heavy editors before the one-step test"
}
...
$Report = Join-Path $RunDir "one_step.report.json"
$Stdout = Join-Path $RunDir "one_step.stdout.txt"
$Stderr = Join-Path $RunDir "one_step.stderr.txt"
$OutputWav = Join-Path $RunDir "one_step.wav"
...
Write-Host "RTX 3060 VOXCPM2 CUDA ONE-STEP" -ForegroundColor Cyan
Write-Host "One short phrase, one inference step, one attempt" -ForegroundColor Yellow
Write-Host "Normal PyTorch caching allocator is enabled" -ForegroundColor Green
...
try {
    ...
    Show-GpuTelemetry "GPU before one-step"
    ...
    if (-not $Process.Start()) {
        throw "Could not start the VoxCPM2 one-step process"
    }
    ...
    Show-GpuTelemetry "GPU after one-step"
    ...
    if ($GpuEvents.Count -gt 0) {
        ...
        $GpuEvents |
            ...
            Format-List
        throw "Windows logged a new GPU/Display/WHEA event during one-step generation"
    }
    if (-not $Completed) {
        throw "One-step generation timed out after $TimeoutSeconds seconds"
    }
    if ($ExitCode -ne 0) {
        Write-Host $StageStderr -ForegroundColor Red
        throw "One-step generation failed with exit code $ExitCode"
    }
    if (-not (Test-Path -LiteralPath $Report)) {
        throw "One-step generation did not write its report"
    }
    $Payload = Get-Content -LiteralPath $Report -Raw | ConvertFrom-Json
    if ($Payload.status -ne "passed") {
        throw "One-step report status was not passed"
    }
    if (-not (Test-Path -LiteralPath $OutputWav)) {
        throw "One-step generation did not create the WAV output"
    }

    Write-Host "" 
    Write-Host "VOXCPM2 CUDA ONE-STEP PASSED" -ForegroundColor Green
