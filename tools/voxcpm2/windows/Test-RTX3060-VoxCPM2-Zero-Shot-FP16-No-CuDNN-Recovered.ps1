...
param(
    ...
    [string]$WorkRoot = "C:\AI-Archive\RTX3060-VOXCPM2-ZERO-SHOT-FP16-NO-CUDNN",
...
$Replacements = [ordered]@{
    'tools\voxcpm2\voxcpm2_cuda_zero_shot_smoke.py' = 'tools\voxcpm2\voxcpm2_cuda_zero_shot_fp16_no_cudnn_compat_smoke.py'
    ...
    'RTX 3060 VOXCPM2 CUDA ZERO-SHOT — RECOVERED SESSION' = 'RTX 3060 VOXCPM2 ZERO-SHOT — FP16 / NO cuDNN'
    'One zero-shot phrase, one inference step, one attempt' = 'One FP16 attempt, one inference step, retries disabled, cuDNN disabled'
}
...
$TempRoot = Join-Path ([IO.Path]::GetTempPath()) "voxcpm2-fp16-no-cudnn"
New-Item -ItemType Directory -Force -Path $TempRoot | Out-Null
$TempScript = Join-Path $TempRoot ("fp16-no-cudnn-{0}.ps1" -f ([guid]::NewGuid().ToString("N")))
