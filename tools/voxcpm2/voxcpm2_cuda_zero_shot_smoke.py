#!/usr/bin/env python3
"""Run one isolated zero-shot VoxCPM2 CUDA generation without reference audio."""
...
def main() -> int:
    ...
    parser.add_argument("--text", default="Тест.")
    ...
    if not 1 <= args.steps <= 4:
        raise RuntimeError("zero-shot smoke steps must be within 1..4")
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "test": "voxcpm2_cuda_zero_shot_smoke",
        "status": "starting",
    ...
    try:
        ...
        print("reference audio: none (zero-shot)", flush=True)
        ...
        report["status"] = "generating_zero_shot"
        write_report(report_path, report)
        print(f"generating zero-shot once: {kwargs}", flush=True)
