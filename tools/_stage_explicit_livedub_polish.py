#!/usr/bin/env python3
"""One-shot branch polish for the explicit LiveDub refactor."""
from pathlib import Path
import re

pipeline = Path("pipelines/main_pipeline.py")
...
if count != 1:
    raise SystemExit(f"expected one stale Quick info block, got {count}")
if "_send_livedub_info_card_once" in text:
    raise SystemExit("dangling _send_livedub_info_card_once remains")
pipeline.write_text(text, encoding="utf-8")

Path("tests/test_explicit_livedub_delivery_transactions.py").write_text(
