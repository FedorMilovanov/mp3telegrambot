...
2. analyses an 8-second central crop, where a real speaker's face/mouth/gestures occupy more of the frame;
3. downsamples with the `area` scaler to 96×96 grayscale, suppressing codec noise, film grain, and tiny particles;
4. combines dominant `freezedetect` coverage with `signalstats` YDIF median/p90/p98 motion evidence;
5. requires enough samples and logs all metrics/reasoning;
6. caches bounded results by file fingerprint and timestamp;
7. fails safely to the existing crop mode on every error or uncertain result.
