...
## 2026-08-13 — Ordinary Shorts 3-minute timing/replay closure

- A post-Factory audit found that ordinary Shorts already scaled Gemini candidate source windows by playback speed, while the render pipeline could add a second speed compensation. Optional boundary padding and silence snapping could also escape the same source budget, and a failed required speed transform could fall back to an overlong raw file.
- Ordinary Shorts now have one request-local duration owner: the semantic source window is bounded by `180s * speed`; optional preroll/postroll is reclaimed inside that budget; silence snapping cannot escape it; semantic candidates beyond the proved source duration fail closed instead of being silently cut.
- Required non-unity speed transforms fail closed if processing fails, and the final selected Telegram artifact must probe at `<=180s` plus the existing 50ms container/frame tolerance. Factory jobs bypass this layer and keep their stricter Factory timing/delivery authority.
- Saved trim/replay timing now uses the actual request-local render window after padding reclamation rather than the stale generic pipeline local, preventing replay/trim controls from drifting from the delivered media. Non-finite optional padding values are sanitized instead of poisoning FFmpeg timing.
...
## 2026-08-14 — Generic montage/normalize video generation-loss closure

- Continued the media-quality audit into ordinary generic Montage/Highlights. Final Montage concat no longer creates a second lossy video generation when its parts share the same encoder/filter contract: compatible video packets are copied while audio is remuxed/re-encoded as AAC. Normalize-only Short postprocessing at actual unity speed likewise applies loudness normalization to audio while packet-copying video; non-unity speed continues through the existing video-transform path.
- The explicit deterministic FFmpeg verifier rendered three real H.264 `720x1280` Montage parts, then exercised the production concat and normalize-only paths. Canonical H.264 bitstream hashing was unchanged across packet-copy concat and normalize-only, reducing the tested pre-subtitle lossy video generations from `3` to `1`. This evidence is intentionally limited to the deterministic H.264/FFmpeg path; subtitle burn remains a video encode and no universal zero-loss claim is made for unrelated codecs/paths.
- A final adversarial pass found that the new generic copy predicate inherited a broad near-unity tolerance that could swallow a small requested speed transform. Packet-copy eligibility now requires effective speed `1.0` to machine-level tolerance; malformed/non-finite values and real non-unity speeds stay off the copy path.
- The clean replacement branch also excluded an unrelated accidental Linux poster-font path regression found in the exploratory branch. The canonical `/usr/share/fonts/truetype/...` fallbacks remain unchanged from `main`.
- `tools/verify_generic_video_generation_loss.py` contains the repeatable local media proof and bootstraps the repository root before importing `services`, so it runs as a standalone verifier. The temporary apt/network GitHub Actions workflow used only to collect one-shot FFmpeg evidence is deliberately not retained in the final PR.
- Existing Factory and verified-Highlights quality owners, public duration/delivery gates, non-unity speed semantics, subtitle behavior and encoder quality profiles are not weakened. Exact-head Python 3.11, Python 3.13, Windows and Cut Policy CI are required before merge.

## 2026-09-07 — Gemini 3.8 project-quota routing and Auth-key operations

- Production retry semantics were re-audited after real `503/high demand` and `429 RESOURCE_EXHAUSTED` evidence. `503` remains backend-capacity/transient handling under the existing bounded global retry budget; `429` remains quota/rate-limit handling. Neither class may multiply retries merely because more API keys are configured.
- Gemini quota identity is treated as a Google Cloud **project/quota-domain** concern rather than an API-key-count concern. The repository never derives project identity from secret key material. Optional `GEMINI_QUOTA_DOMAIN[_2.._4]` values are opaque local relationship labels configured only after the owner verifies the real key-to-project mapping outside the repository.
- Same-domain sibling suppression is allowed only after an explicitly project-scoped quota response. Generic `429 RESOURCE_EXHAUSTED` remains conservative and rotates normally. Unknown, custom or misaligned client metadata fails open to the legacy bounded credential route instead of guessing identity.
- The project-aware contract now covers the common global router plus specialized direct transports that must preserve Files/upload affinity: Factory, main audio analysis, LiveDub QA, Quiz, translation editorial review, publication/title/subtitle helpers and VoxCPM translation. Files and inference quota scopes remain independent where the provider exposes separate operations.
- `gemini_analyze` LOW-thinking and timestamp-coverage recovery remain intentionally pinned one-shot calls on the already uploaded file/client. Cross-client rotation there would require a new upload and a new recovery budget, so this affinity is explicit and regression-tested rather than hidden as an exception.
- Semantic-invalid successful HTTP responses can be rejected by a caller validator and rotate within the same existing global attempt budget; validation cannot expand the retry budget and does not mark a quota domain exhausted.
- Added a fail-closed exact-tree AST census for Gemini SDK entry points and client creation. Every executable occurrence is classified as central router, specialized project-aware transport, intentionally pinned same-client recovery or test/config-only; unclassified runtime routes fail CI. The census also watches streaming generation, token-count and embedding calls so new quota-consuming surfaces cannot bypass ownership silently.
- Gemini QA workflow ownership was widened to compile/test the real direct-caller surface. Final routing work also removed duplicated response-parser regex logic instead of raising the code-health baseline; the measured regex delta moved below the existing baseline.
- No API keys, Google Cloud project IDs, billing identifiers or raw quota-domain labels are committed or logged. Owner issue #184 remains the private operational step: migrate any remaining Standard keys to Auth keys, verify project/tier relationships in AI Studio/Cloud, configure only opaque local domain labels, then exercise both `generateContent` and Files API paths before revoking old credentials.
...
- VTT lines inside one cue are now treated as adjacent render states rather than blindly concatenated. Growing/duplicate states collapse to the complete text while separated rhetorical repetition remains intact; this aligns Synopsis parsing with the already source-owned Dub Studio parser contract.
...
- Telegraph create/edit field limits are centralized in `core.telegraph_contract`; every low-level transport clamps composed titles after prefixes/suffixes, closing the residual `Вопросы: <long title>` failure path. Recursive part/TOC titles reserve their structural suffix before truncation so part identity is not lost.
- Generic Telegraph create now retries bounded transient network failures and transient HTTP 408/425/429/5xx responses, including non-JSON 5xx pages, while `FLOOD_WAIT_N` retains its exact server-directed wait. Deterministic API failures are still not retried.
- Removed the generic recursive Telegraph `depth < 4` split cap. Each CONTENT_TOO_BIG split strictly reduces node count and terminates at one unsplittable node, so very large Synopsis fallback output is no longer exposed to an arbitrary maximum-parts/data-loss boundary.
- Removed the separate 12-page/default and 50-page/hard caps from multipart Telegraph chain expansion in production repair and the CLI audit tool. Both now traverse the full `➡ Дальше` chain by default with cycle protection; an explicit caller-supplied limit remains available for diagnostics. This prevents repair/audit from silently declaring success after inspecting only the first part of a longer legal Synopsis chain.
- Cache invalidation now has an explicit generation/render dependency manifest covering persisted analysis, prompts, audits, structured blocks, title/timestamp policy, Study synthesis and Telegraph transports. The manifest is regression-checked for duplicates and nonexistent paths.
- Semantic environment switches that alter persisted analysis/page content are now part of `prompt_version` (structured analysis/pages, timestamp repair, Synopsis prompt/transcript policy, expanded-page retry/combined mode). Pure timeout/backoff knobs remain excluded so availability tuning does not invalidate semantically identical content.
- The fingerprint dependency closure also includes `services/search.py` because `_build_search_title()` feeds public Telegraph titles/alternate-link cache fields, and `services/telegraph_repair.py` plus `TELEGRAPH_AUTO_REPAIR_AFTER_PUBLISH` because deterministic post-publish repair changes the persisted public page. This avoids hashing the entire operational `main_pipeline.py` while still covering its direct semantic generation owners.
- LiveDub translation QA no longer consumes `adb_get(media_id)["ai_data"]` without validation. Cached analysis is used as a fallback semantic reference only after normal TTL/cache_version/prompt_version/model validation; stale analysis is ignored and logged instead of influencing QA/autofix when original audio is unavailable.
