...
def test_lazy_session_opens_only_for_length_planning() -> None:
    ...
    class Session:
        audio_spec = Spec()
        def generate(self, request): return request

    class Backend:
        backend_id = "voxcpm2"
        def capabilities(self): return SimpleNamespace(continuation_context=True)
        def open_session(self, config): calls.append("open"); return Session()
        def plan_generation_length(self, spec, request): return spec.seconds_per_step
        def plan_generation_profile(self, request): return request
...
def test_reference_cache_requires_full_evidence(tmp_path: Path) -> None:
    ...
    (output.parent / "references.json").write_text(
        json.dumps({"extended": entry}), encoding="utf-8"
    )
    assert io.cached_reference(
        source=source, output=output, hash_file=digest, expected_sample_rate=16000
    )["reference_cache_hit"] is True
