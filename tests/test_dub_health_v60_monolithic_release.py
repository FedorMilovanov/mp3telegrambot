...
def test_release_contract_replaces_only_superseded_legacy_checks() -> None:
    ...
    assert ok is True, detail
    assert WORKER_RUNTIME in detail
    assert dub_title_policy.RELEASE_CONTRACT_POLICY in detail
...
def test_shared_worker_runtime_and_policies_are_current() -> None:
    ...
    assert BACKEND_ENVIRONMENT_POLICY == "speech-backend-environment-policy-v1"
    assert MONOLITHIC_VOICE_POLICY == "single-speaker-monolithic-candidate-v1"
    assert SOURCE_RELATIVE_CONTINUITY_POLICY == (
        "cross-language-source-prosody-diagnostic-v3"
    )
    assert FAIL_CLOSED_IDENTITY_POLICY == (
        "cross-language-prosody-cannot-override-identity-v1"
    )
    ...
    assert PRONUNCIATION_VARIANT_POLICY == (
        "bounded-pronunciation-candidate-variants-v1"
    )
