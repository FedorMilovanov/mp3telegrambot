...
def test_r39_vtt_substring_word_not_dropped():
    ...
    yt._append_caption_delta(chunk, "king")            # «king» — подстрока «kingdom», но отдельное слово
    assert "king" in chunk
    # точный дубль всё ещё отбрасывается
...
def test_r39_dur_score_both_unknown_neutral():
    ...
    assert unknown < perfect                           # unknown не должен равняться идеальному матчу
...
def test_r39_qa_info_survive_string_shapes():
    ...
    assert format_qa_report({"score": 90, "issues": ["10:05 wrong", "bad"]})   # не падает
    # info с scripture списком строк
...
def test_r39_archive_rows_coerce_list_elements():
    ...
    assert out["hashtags"] == ["1", "2"]               # int → str, без краха " ".join
...
def test_r39_telegraph_destructive_move_removed():
    for f in ("services/telegraph.py", "services/telegraph_pages.py"):
        ...
        assert "переношу последнюю секцию" not in src   # опасный перенос убран


def test_r39_mp3_reencode_guarded_against_self():
    src = Path("pipelines/main_pipeline.py").read_text(encoding="utf-8")
    assert src.count("mp3_path.name != mp3_64_path.name") >= 2   # оба пути защищены


def test_r39_fallback_marker_read_live():
    src = Path("pipelines/main_pipeline.py").read_text(encoding="utf-8")
    assert "import services.telegraph_pages as _tp_module" in src
    assert "_tp_module._gemini_last_was_fallback" in src
    # старый импорт-по-значению убран
    assert "    _gemini_last_was_fallback,\n)" not in src
