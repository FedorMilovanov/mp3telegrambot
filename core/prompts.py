...
1. НЕ оборачивай всю карточку в **жирный** — автор и название пишутся обычным текстом.
   Русское название — в *курсиве* в «ёлочках». Английское — в *курсиве* в скобках.
   Пример: Джон Оуэн, *«Умерщвление греха»* (*John Owen, Of the Mortification of Sin*)
   Пример: 1689 Лондонское баптистское исповедание, главы 10, 13, 15
   Точку в конце строки заголовка НЕ ставить.
...
- заголовков scripture-блоков (ссылка стоит ОТДЕЛЬНОЙ строкой как заголовок микро-блока): **Матфея 7:21–23**, **Исаия 6:1–5**
...
Формат каждого пункта (БЕЗ маркеров • или тире перед вопросом):
**Конкретный вопрос — привязанный к сцене или моменту из проповеди.**
Раскрытие (2–4 предложения) следующим абзацем через \n\n:
сценарий из проповеди → почему это болит → куда ведёт.

ЗАПРЕЩЕНО: начинать с «• **Вопрос**» или «- **Вопрос**» — только голый **жирный** на отдельной строке.
...
• **От паники к упованию на Божью заботу.**  Многие верующие родители годами живут
  в страхе за вечную участь детей, не понимая, как суверенная благодать действует
  в отношении тех, кто не способен сделать осознанный выбор [⏱ 0:00].
  Эта проповедь меняет угол зрения, раскрывая *возраст ответственности*
  (Age of Accountability) — период, до которого дети под защитным покровом Творца [⏱ 29:32].
