...
Почему именно это понятие богословски значимо в данном контексте проповеди.
...
НЕ ставь эмодзи в конце заголовка. Вместо этого поставь эмодзи ПЕРВЫМ СИМВОЛОМ
в поле "content" (не в title).

ПРАВИЛЬНО:
- title: "Ключевые понятия и Doctrinal Stakes"
- content начинается с: "🔑 [текст]"

НЕПРАВИЛЬНО:
- title: "Ключевые понятия и Doctrinal Stakes 🔑"
(Причина: Telegraph оборачивает title в <h4>, и эмодзи в конце
переносится на отдельную строку при узком viewport.)

Исключение: если title короче 25 символов — эмодзи в конце допустим.
...
7. НЕ используй конструкции «🏛️ **-** текст **.**» — пиши просто «🏛️ текст.»
...
КРИТИЧЕСКИ ВАЖНО — ИМЕНИТЕЛЬНЫЙ ПАДЕЖ:
**«Русское название»** ВСЕГДА пишется в именительном падеже — независимо от контекста предложения.
Если предложение «тянет» термин в другой падеж — перестрой предложение вокруг термина, но не склоняй сам термин.

❌ ЗАПРЕЩЕНО (генитив): «концепция **«Упреждающей благодарности»**» — термин склонён
❌ ЗАПРЕЩЕНО (датив): «обратиться к **«Духовной литургии времени»**» — термин склонён
✅ ПРАВИЛЬНО: «это и есть **«Упреждающая благодарность»** (**Anticipatory Gratitude**)»
✅ ПРАВИЛЬНО: «Сперджен называет это **«Духовная литургия времени»** (**Spiritual Liturgy of Time**)»
