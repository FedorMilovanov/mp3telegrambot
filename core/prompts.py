...
5. «Карта источников»: заголовки (автор+книга) — ОБЫЧНЫМ шрифтом. Термины в описаниях — **жирным**.
...
Раскрытие (2–4 предложения): сценарий из проповеди → почему это болит → куда ведёт.
...
3–4 контраста (не 5–6 — иначе превращается в однообразный список).
Только те, что прямо вытекают из проповеди.

КРИТИЧЕСКИ ВАЖНО — СКОБКИ И ЖИРНЫЙ:
Если рядом с жирным словом стоят скобки с английским термином — весь блок ОДНИМ жирным:
✅ **«Терапевтическое Евангелие» (Therapeutic Gospel)**
❌ **«Терапевтическое Евангелие»** ( **Therapeutic Gospel**) — ломает рендер
❌ **«Терапевтическое»** (Therapeutic) **Евангелие** — разбитый bold
Правило: между открывающим ** и закрывающим ** не должно быть других ** маркеров.

КРИТИЧЕСКИ ВАЖНО — ФОРМАТИРОВАНИЕ ЗАГОЛОВКА ❌/✅:
Если в заголовке есть английский термин в скобках — включай его ВНУТРЬ жирного,
без разрыва ** маркеров внутри строки.
❌ ЗАПРЕЩЕНО: **«Лёгкое верие» (** Easy Believism **) как...** — ломает рендер!
✅ ПРАВИЛЬНО:  **«Лёгкое верие» (Easy Believism) как...** — весь заголовок одним bold-блоком.
Правило: между открывающим ** и закрывающим ** не должно быть других ** маркеров.

ВАЖНО — ГРУППИРОВКА ПУНКТОВ:
Каждая пара ❌/✅ образует визуальный блок. Между парами — пустая строка.
НЕ перечисляй все ❌ подряд, а затем все ✅ — это разрушает связь пар.
Порядок всегда: ❌ заблуждение → ✅ ответ → (пустая строка) → следующая пара.
...
СИЛЬНО (живой молитвенный абзац):
1. **Молитва об обнажении скрытого бунта.**
Господи, я слышал о человеке, который годами казался послушным —
но только потому, что боялся земного отца. Сегодня я признаю,
что многое из моей «праведности» держится не на страхе пред Тобой,
а на страхе перед людьми и их мнением. Покажи мне конкретно —
где я служу напоказ. Где улыбаюсь в церкви, но не разговариваю
с Тобой дома. Разрушь этот фасад — мне больше нечем за него прятаться.
