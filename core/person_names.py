...
_PERSON_REPLACEMENTS: tuple[tuple[str, str], ...] = (
    ...
    # AUDIT R23 (Source Card: "Holiness" без перевода "Святость" — Gemini
    # написал инициал через "К", а не через "Ч"): "C" в англ. инициалах
    # неоднозначно транслитерируется (К или Ч) — тот же класс ошибки, что
    # уже защищён у Спроула выше. Без этого original_author_name() не может
    # свести "Дж. К. Райл" обратно к "J.C. Ryle" и official_ru_title()
    # молча теряет уже известный официальный перевод названия книги.
    ("Дж. К. Райл", "Дж. Ч. Райл"),
